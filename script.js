...
document.addEventListener('DOMContentLoaded', () => {
    async function migrateIfNeeded() {
        try {
            ...
            const backupLegacy = (key, raw) => {
                if (!raw) return;
                const stamp = new Date().toISOString().replace(/[:.]/g, '-');
                try { localStorage.setItem(`legacy_backup_${key}_${stamp}`, raw); } catch {}
                try { localStorage.removeItem(key); } catch {}
            };
            const songsParsed = safeParseFromStorage('songs', []);
            const setlistsParsed = safeParseFromStorage('setlists', []);
            if (!songsParsed.valid) backupLegacy('songs', songsParsed.raw);
            if (!setlistsParsed.valid) backupLegacy('setlists', setlistsParsed.raw);
            const songs = songsParsed.value;
            const setlists = setlistsParsed.value;
            ...
            try { localStorage.removeItem('songs'); } catch {}
            try { localStorage.removeItem('setlists'); } catch {}
  ...
  const app = {
        normalizeTitle(title) {
            return normalizeSongTitleValue(title);
        },

        cleanLyricsForStorage(title, lyrics) {
            return normalizeLyricsBlock(title, lyrics);
        },
        ...
        tabToolbars: {
            songs: `
                <div class="search-group">
                  ...
                  <button id="song-search-clear" class="search-clear-btn" type="button" title="Clear search" aria-label="Clear search">
                ...
                <div class="toolbar-buttons-group">
                    <label title="Favorites only" class="btn" style="display:inline-flex;align-items:center;gap:0.35rem;">
            ...
            performance: `
                ...
                <div class="search-group no-sort">
                  ...
                  <button id="performance-search-clear" class="search-clear-btn" type="button" title="Clear search" aria-label="Clear search">
                    <i class="fas fa-times-circle" aria-hidden="true"></i>
                  </button>
                </div>
        ...
        renderToolbar(tab) {
            ...
            if (tab === 'songs') {
                ...
                const songClearBtn = document.getElementById('song-search-clear');
                this.attachSearchClear(this.songSearchInput, songClearBtn);
                ...
                const favFilter = document.getElementById('song-fav-filter');
                if (favFilter) {
                    favFilter.addEventListener('change', () => {
                        localStorage.setItem('songsFavoritesOnly', favFilter.checked ? '1' : '0');
                        this.renderSongs();
                    });
                }
                ...
                this.deleteAllSongsBtn.addEventListener('click', () => {
                    confirmDialog('Delete ALL songs? This cannot be undone!', async () => {
                        ...
                        await this.saveData();
                        const impactedSetlists = SetlistsManager.getAllSetlists().filter(sl => Array.isArray(sl.songs) && sl.songs.length);
                        for (const sl of impactedSetlists) {
                            SetlistsManager.updateSetlistSongs(sl.id, []);
                        }
                        if (impactedSetlists.length) this.renderSetlists();
                        this.renderSongs();
                        const extras = impactedSetlists.length ? ` Cleared ${impactedSetlists.length} setlist(s).` : '';
                        showToast(`Deleted all songs.${extras}`, 'success');
                    });
                });
                ...
                const perfClearBtn = document.getElementById('performance-search-clear');
                this.attachSearchClear(this.performanceSongSearch, perfClearBtn);
        ...
        async handleImportSetlistImage(event) {
            try {
                ...
                const tessBase = (() => {
                    try {
                        return new URL('lib/tesseract/', window.location.href).href.replace(/\/$/, '');
                    } catch {
                        return `${window.location.origin}/lib/tesseract`;
                    }
                })();
        ...
        attachSearchClear(inputEl, btnEl) {
            if (!inputEl || !btnEl) return;
            const toggle = () => {
                const hasValue = inputEl.value && inputEl.value.length > 0;
                btnEl.classList.toggle('visible', hasValue);
            };
            toggle();
        ...
        async init() {
            ...
            this.loadData();
            // Determine initial tab from URL hash or saved preference
            const hashTab = (location.hash || '').replace('#','');
            const savedTab = (localStorage.getItem('lastActiveTab') || '').trim();
            const initialTab = ['songs','setlists','performance'].includes(hashTab)
                ? hashTab
                : (['songs','setlists','performance'].includes(savedTab) ? savedTab : 'songs');
            this.renderToolbar(initialTab);
            this.setlistSelect = document.getElementById('setlist-select');
            this.performanceSetlistSelect = document.getElementById('performance-setlist-select');
            ...
            try {
                if (initialTab !== 'songs') {
                    const btn = Array.from(this.navButtons).find(b => b.dataset.tab === initialTab);
                    btn?.click();
                }
            } catch {}
            // Load from IDB
            try {
                const rawSongs = await DB.getAllSongs();
                const { sanitizedSongs, changed } = this.sanitizeSongs(rawSongs || []);
                this.songs = sanitizedSongs;
                if (changed) {
                    try { await DB.putSongs(this.songs); } catch (err) {
                        console.error('Failed to persist song cleanups', err);
                    }
                }
            } catch (e) {
            ...
            // Render content for chosen tab
            if (initialTab === 'songs') this.renderSongs();
            if (initialTab === 'setlists') this.renderSetlists();
            if (initialTab === 'performance') this.renderPerformanceTab();
            try {
            ...
            try { this.checkBackupReminder(); } catch {}
        ...
        normalizeSongRecord(entry) {
            const clone = { ...(entry || {}) };
            let changed = false;
            const normalizedTitle = normalizeSongTitleValue(clone.title || '');
            const safeTitle = normalizedTitle || 'Untitled';
            if (safeTitle !== (clone.title || '')) {
                clone.title = safeTitle;
                changed = true;
            }
            const normalizedLyrics = normalizeLyricsBlock(clone.title, clone.lyrics || '');
            if (normalizedLyrics !== (clone.lyrics || '')) {
                clone.lyrics = normalizedLyrics;
                changed = true;
            }
            return { song: clone, changed };
        },

        sanitizeSongs(rawSongs) {
            const sanitized = [];
            let changed = false;
            for (const entry of rawSongs || []) {
                if (!entry || typeof entry !== 'object') continue;
                const { song, changed: songChanged } = this.normalizeSongRecord(entry);
                sanitized.push(song);
                if (songChanged) changed = true;
            }
        ...
        setupEventListeners() {
            const activateTab = (btn) => {
                ...
                this.navButtons.forEach(b => { b.classList.remove('active'); b.setAttribute('aria-selected','false'); b.setAttribute('tabindex','-1'); });
            ...
            this.navButtons.forEach((btn, idx) => {
                ...
                btn.addEventListener('keydown', (e) => {
                    if (e.key === 'ArrowRight' || e.key === 'ArrowLeft') {
                        e.preventDefault();
                        const buttons = Array.from(this.navButtons);
            ...
            if (exportModal) {
                ...
                reminderToggle?.addEventListener('change', () => {
                    this.saveBackupReminderSettings(reminderToggle.checked, reminderDays?.value);
                });
        ...
        showEditChoice(id) {
            ...
            modal.className = 'modal';
            modal.style.display = 'flex';
            const s = this.getLyricById(id);
            const isFav = !!(s && s.favorite);
            const favLabel = isFav ? 'Remove Favorite' : 'Add to Favorites';
            modal.innerHTML = `
            ...
            const delBtn = modal.querySelector('#delete-song');
            if (delBtn) {
                delBtn.onclick = () => { close(); this.deleteSong(id); };
            }
        ...
        loadBackupReminderSettings() {
            const enabled = localStorage.getItem('backupReminderEnabled') === '1';
            const days = Number(localStorage.getItem('backupReminderDays')) || 30;
            this.backupReminderEnabled = enabled;
            this.backupReminderDays = days;
            const toggle = document.getElementById('backup-reminder-toggle');
            const select = document.getElementById('backup-reminder-days');
            if (toggle) toggle.checked = enabled;
            if (select) select.value = String(days);
        },

        saveBackupReminderSettings(enabled, days) {
            this.backupReminderEnabled = !!enabled;
            this.backupReminderDays = Number(days) || 30;
            localStorage.setItem('backupReminderEnabled', this.backupReminderEnabled ? '1' : '0');
            localStorage.setItem('backupReminderDays', String(this.backupReminderDays));
        },

        checkBackupReminder() {
            if (!this.backupReminderEnabled) return;
            if (sessionStorage.getItem('backupReminderShown') === '1') return;
            const last = Number(localStorage.getItem('lastExportAt')) || 0;
            const ageDays = last ? (Date.now() - last) / (1000 * 60 * 60 * 24) : Infinity;
            if (ageDays >= this.backupReminderDays) {
                showToast('Backup reminder: export a full backup to keep your data safe.', 'info', 4500);
            ...
            try {
                ...
                if (what === 'everything') {
                    ...
                    if (format === 'pdf') {
                        ...
                        if (what === 'all-setlists' && format === 'json') isFullBackup = true;
                    }
                }
        ...
        exportSongsPDF() {
            ...
            htmlParts.push(`<style>
                ...
                .song { break-before: page; page-break-before: always; font-size: 14pt; text-align: left; }
                .song:first-child { break-before: auto; page-break-before: auto; }
                ...
                @media screen { body { background:#f5f5f5; } .song { box-sizing: border-box; width: 8.27in; margin: 12px auto; background:#fff; border: 1px solid #ddd; padding: 0.75in; } }
            </style></head><body>`);
        ...
        async importSongs(fileList, dupMode) {
            ...
            for (const file of fileList) {
                if (/\.json$/i.test(file.name)) {
                    ...
                    // - Object with { songs: [...] } (library export or full backups)
        ...
        async mergeSongsByTitle(incoming, dupMode) {
            ...
            for (const s of incoming) {
                ...
                const rawSong = { id: s.id || (Date.now().toString()+Math.random().toString(16).slice(2)), title, lyrics: s.lyrics||'' };
                const { song: normalizedSong } = this.normalizeSongRecord(rawSong);
                this.songs.push(normalizedSong);
                existing.set((normalizedSong.title || '').trim().toLowerCase(), normalizedSong);
                await DB.putSong(normalizedSong);
            }
            await this.saveData();
        },
        ...
        renderSongs() {
            ...
            const favOnly = (localStorage.getItem('songsFavoritesOnly') === '1');
            if (favOnly) filteredSongs = filteredSongs.filter(s => !!s.favorite);
            filteredSongs = this.applySongSort(filteredSongs);
            this.songList.innerHTML = filteredSongs.map(song => `
                <div class="song-item" data-id="${song.id}">
                    <span class="song-title">${escapeHTML(song.title)}</span>
                    <div class="song-actions">
                        <button class="song-menu-btn" title="Song options" aria-label="Song options">
            ...
            // No inline delete or favorite here; both are in the edit options modal now.
                ...
                pressTimer = setTimeout(async () => {
                    ...
                    try {
                        if (navigator.clipboard && window.isSecureContext) {
                            ...
                            const ta = document.createElement('textarea');
                            ta.value = text; ta.className = 'visually-hidden-clipboard';
                            document.body.appendChild(ta); ta.focus(); ta.select();
                            document.execCommand('copy'); ta.remove();
                        }
                        showToast('Copied song (title + lyrics).', 'success');
        ...
        async saveSong() {
            ...
            if (!title) return;
            const sanitizedLyrics = this.cleanLyricsForStorage(title, lyrics);
            if (this.currentSongId) {
                const song = this.songs.find(s => s.id === this.currentSongId);
                if (!song) return;
                song.title = title;
                song.lyrics = sanitizedLyrics;
                song.lastEditedAt = new Date().toISOString();
                const { song: normalizedSong } = this.normalizeSongRecord(song);
                Object.assign(song, normalizedSong);
                ...
                const { song: normalizedSong } = this.normalizeSongRecord(newSong);
        ...
        deleteSong(id) {
            ...
            confirmDialog(msg, async () => {
                const song = this.getLyricById(id);
                const affected = inSetlists.map(s => s.id);
                await this.removeLyric(id);
                inSetlists.forEach(s => SetlistsManager.removeSongFromSetlist(s.id, id));
                this.renderSongs();
                this.renderSetlists();
                let undone = false;
                showActionToast(count ? `Deleted song and updated ${count} setlists.` : 'Deleted song.', {
                    type: 'success',
                    actionText: 'Undo',
                    onAction: async () => {
                        if (undone) return; undone = true;
                        if (song) {
                            try { await DB.putSong(song); } catch {}
                            this.songs.push(song);
                            for (const sid of affected) {
                                SetlistsManager.addSongToSetlist(sid, song.id);
                            }
                            await this.saveData();
                            this.renderSongs();
                            this.renderSetlists();
        ...
        renderSetlists() {
            ...
            if (this.performanceSetlistSelect) {
                let prev = this.performanceSetlistSelect.value || this.performanceSetlistId || '';
                // Restore from last performance or last active selection when available
                if (!prev) {
                    try {
                        const lpRaw = localStorage.getItem('lastPerformance');
                        const lp = lpRaw ? JSON.parse(lpRaw) : null;
                        if (lp && lp.setlistId) prev = lp.setlistId;
                        const las = localStorage.getItem('lastActiveSetlistId');
                        if (!prev && las) prev = las;
                    } catch {}
                }
        ...
        handlePerformanceSetlistChange() {
            ...
            try { localStorage.setItem('lastActiveSetlistId', this.performanceSetlistId || ''); } catch {}
        ...
        startPerformanceWithSong(songId) {
            ...
            // Choose a setlist if none selected: prefer last active setlist containing the song,
            // then any setlist containing it.
            let chosenSetlistId = this.performanceSetlistId || null;
            if (!chosenSetlistId) {
                try {
                    const las = localStorage.getItem('lastActiveSetlistId');
                    if (las) {
                        const sl = SetlistsManager.getSetlistById(las);
                        if (sl && Array.isArray(sl.songs) && sl.songs.includes(songId)) chosenSetlistId = las;
                    }
                } catch {}
                if (!chosenSetlistId) {
                    const all = SetlistsManager.getAllSetlists();
                    const found = all.find(sl => Array.isArray(sl.songs) && sl.songs.includes(songId));
                    if (found) chosenSetlistId = found.id;
                }
            }
            if (chosenSetlistId) {
                params.set('setlistId', chosenSetlistId);
                try { localStorage.setItem('lastActiveSetlistId', chosenSetlistId); } catch {}
                const setlist = SetlistsManager.getSetlistById(chosenSetlistId);
