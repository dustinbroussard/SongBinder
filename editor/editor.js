// Initialize theme before DOM (CSP-safe, no inline)
try {
  const t = localStorage.getItem('theme') || 'dark';
  document.documentElement.dataset.theme = t;
} catch {}

// Register SW (editor is a separate entry)
try {
...
async function ensurePersistentStorage() {
    try {
        ...
        if (!granted) {
            console.warn('Persistent storage request was denied; edits may be evicted under pressure.');
        }
    } catch (err) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('touchstart', (e) => {
        ...
        if (!btn) return;
        const scoped = btn.closest('#app-header, .modal-overlay, .ai-context-menu, .section-menu, #font-controls');
        if (!scoped) return;
        e.preventDefault();
        btn.click();
    }, { passive: false });
    ...
    // Reflect meta theme-color
    try { setThemeColorMeta(document.documentElement.dataset.theme || 'dark'); } catch {}
...
// Clipboard Manager Class
    ...
    // Metadata panel UX wiring (moved from inline script for CSP)
    (function wireMetadataPanel(){
        ...
        saveBtn?.addEventListener('click', () => {
            if (window.app?.saveCurrentSong) window.app.saveCurrentSong(true);
        });
        document.addEventListener('click', (e) => {
            if (window.innerWidth <= 800 && metadataPanel?.classList.contains('open') && !metadataPanel.contains(e.target) && !metadataToggleBtn?.contains(e.target)) {
                metadataPanel.classList.remove('open');
            }
        });
    })();
...
async function callOpenRouterAPI(prompt) {
    try {
        ...
        if (typeof ClipboardManager !== 'undefined' && ClipboardManager.showToast) {
            ClipboardManager.showToast(`AI request failed: ${err.message || err}`, 'error');
        } else {
...
function enforceAlternating(lines) {
    ...
    const app = {
        ...
        forgetAiKeyBtn: document.getElementById('forget-ai-key'),
        ...
        isBlankLineGroup(group) {
            if (!group) return false;
            const lyric = group.querySelector('.lyric-text')?.textContent.trim() || '';
            const chord = group.querySelector('.chord-line')?.textContent.trim() || '';
            return lyric === '' && chord === '';
        },

        ensureSectionGaps() {
            const sections = Array.from(this.lyricsDisplay.querySelectorAll('.section'));
            sections.forEach((section, idx) => {
                const content = section.querySelector('.section-content');
                if (!content) return;
                const isLastSection = idx === sections.length - 1;
                const lastGroup = content.querySelector('.lyrics-line-group:last-child');
                if (!isLastSection) {
                    if (this.isBlankLineGroup(lastGroup)) {
                        lastGroup?.setAttribute('data-section-gap', 'true');
                        lastGroup?.classList.add('section-gap-line');
                    } else {
                        const gapLine = this.addLyricLine('', '', null, 0, content);
                        gapLine.setAttribute('data-section-gap', 'true');
                        gapLine.classList.add('section-gap-line');
                    }
                } else if (lastGroup && lastGroup.dataset.sectionGap === 'true') {
                    if (sections.length === 1) {
                        lastGroup.removeAttribute('data-section-gap');
                        lastGroup.classList.remove('section-gap-line');
                    } else {
                        lastGroup.remove();
                    }
                }
        ...
        setupEventListeners() {
            ...
            this.forgetAiKeyBtn?.addEventListener('click', () => {
                try { localStorage.removeItem('openrouterApiKey'); } catch {}
                try { localStorage.removeItem('openrouterModel'); } catch {}
                window.CONFIG = window.CONFIG || {};
                window.CONFIG.openrouterApiKey = '';
                window.CONFIG.defaultModel = '';
                this.selectedModel = '';
                if (this.apiKeyInput) this.apiKeyInput.value = '';
                if (this.modelSearchInput) this.modelSearchInput.value = '';
                if (this.modelList) this.modelList.innerHTML = '';
                ClipboardManager?.showToast?.('API key cleared.', 'info');
            });
        ...
        saveAISettings() {
            ...
            if (key && !localStorage.getItem('openrouterKeyWarningShown')) {
                try {
                    ClipboardManager.showToast('API key is stored locally on this device.', 'info', 3500);
                    localStorage.setItem('openrouterKeyWarningShown', '1');
                } catch {}
            }
            this.aiSettingsPanel.style.display = 'none';
        },
    ...
    async saveCurrentSong(isExplicit = false) {
        ...
        displayCurrentEditorSong() {
            ...
            if (this.measureModeToggle) this.measureModeToggle.checked = this.isMeasureMode;
        ...
        autoNumberVerses() {
            ...
            this.lyricsDisplay.querySelectorAll('.section-label').forEach(label => {
                ...
                const match = text.match(/^\[\s*verse(?:\s*(\d+))?\s*\]$/i);
                if (!match) return;
                ...
                nextNumber += 1;
                span.textContent = `[Verse ${nextNumber}]`;
            });
        },
    ...
    app.init();
    try {
        if (window.EditorDB?.wasReset?.() && !sessionStorage.getItem('dbResetToastShown')) {
            ClipboardManager?.showToast?.('Storage was reset; please re-import your backup.', 'info', 5000);
